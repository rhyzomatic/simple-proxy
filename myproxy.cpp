...
string rec_header(int client_socket, size_t max_length = 1000){
  ...
  while (total_rec_length < max_length){
    ...
    if (rec_char < 1){
      ...
      total_rec_length++; //is this the same as current_ptr???
      current_ptr++;
      if (strcmp((payload + total_rec_length - 4), "\r\n\r\n") == 0){
          good_header = true;
        break;
      }
    }
...
int get_content_length(string header){
  ...
  if (content_length_location == -1){ // not found, must be 0
    return 0;
  }
  else {
...
void parse_client_header(int client_socket, string header){
  if (header.substr(0,3) != "GET"){
    ...
    if (IMS_location != -1){ //TODO: this is untested
